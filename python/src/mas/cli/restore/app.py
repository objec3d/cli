...
class RestoreApp(BaseApp):
    ...
    #
    # NOTE: This auto-substitution is gated on whether the user has explicitly
    # opted in/out of restoring DRO config from the backup. Without the gate
    # this function silently clobbered an explicit `--include-drocfg-from-backup`,
    # which produced a hard-to-diagnose failure in the suite_restore Ansible
    # role:
    #
    #   1. install_dro creates the `dro-apikey` Secret with `type: Opaque`.
    #   2. The auto-substituted dro.yml manifest declares `type: opaque`
    #      (lowercase). Secret.type is immutable, so the role's
    #      "Apply DRO config file" task fails with a 422 from kube-apiserver:
    #          Secret "dro-apikey" is invalid: type: Invalid value: "opaque":
    #          field is immutable
    #
    # By honouring an explicit `--include-drocfg-from-backup`, the role's
    # "Apply DRO config file" task is short-circuited (its `when` clause is
    # `not (include_dro_from_backup | bool)`), BasCfg is restored from the
    # backup snapshot instead, and we sidestep the immutability collision.
    def setNonInteractiveDROParams(self):
        # `self.args.include_drocfg_from_backup` is None only when the user
        # passed neither --include-drocfg-from-backup nor
        # --exclude-drocfg-from-backup (relies on `default=None` for the shared
        # dest in argParser.py). In that case we preserve the original
        # behaviour: redirect the role at the install-task-generated dro.yml
        # and disable the from-backup path. Otherwise we respect the caller's
        # explicit choice.
        user_specified_drocfg_from_backup = (
            getattr(self.args, "include_drocfg_from_backup", None) is not None
        )
        if self.getParam("include_dro") == "true" and not user_specified_drocfg_from_backup:
